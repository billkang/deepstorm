# capability-assembly Specification

## Purpose

Defines the capability assembly standards and mechanisms for DeepStorm tool packages. Establishes the source directory structure convention (`skills/agents/hooks/mcp` under each `packages/<tool>/`), SKILL.md frontmatter spec (`deepstorm.tool/configKey/configValue/dependencies` fields), `wizard.json` question format, `registry.json` generation via `build-registry.ts`, and the assembly engine that copies/renders assets to `.claude/` at setup time. Supports template rendering (Handlebars), variants selection, and fragment-based multi-dimensional code-style composition.
## Requirements
### Requirement: 源目录结构标准
