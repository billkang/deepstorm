...
### Requirement: questionnaire multiselect/group 类型可测试
`runQuestionnaire` 的 `multiselect` 和 `group` 类型 SHALL 可被单元测试覆盖。
...
- **THEN** `listTemplates()` 被调用时工具参数为 `undefined`
