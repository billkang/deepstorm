...
Defines how the `plugin build` command generates MCP-related configuration files (`.env.example`, `.mcp.json`) in the plugin output directory. Each selected MCP service should produce the corresponding env stubs in `.env.example` and, where applicable, a server definition in `.mcp.json`.

## Requirements

### Requirement: 插件构建生成环境变量模板

`plugin build` 输出的 `.env.example` 文件中 SHALL 包含用户通过交互式向导所选择的所有 MCP 服务对应的环境变量模板。

#### Scenario: 选中 figma/github/jira/context7 时生成对应的 env stub
- **WHEN** 用户运行 `plugin build` 并选中 figma、github、jira、context7 作为 MCP 服务
- **THEN** 生成的 `.deepstorm/plugins/deepstorm/.env.example` 文件中 SHALL 包含以下环境变量模板：
  - `FIGMA_API_KEY`
