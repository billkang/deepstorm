...
Superpowers check gate 是 `reef-start` 在 SDD 文档生成完成后、进入 TDD 实现前的硬性门禁。它强制 AI 在实现前加载适用的 superpowers 技能（如 `superpowers:test-driven-development`），进行 Rigid 纪律声明和用户确认，并包含检查清单和 Red Flags 自省机制。
