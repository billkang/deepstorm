...
- **WHEN** frontmatter 声明 `knowledge_base` 依赖 domain `knowledge-base`，且 `installedMcpServers` 包含 `dingtalk-wiki`（domain = knowledge-base）
- **THEN** 生成的 JSON 中 `knowledge_base` 数组包含 `{"id":"dingtalk-wiki","name":"钉钉云文档","skill":"mcp-dingtalk-wiki"}`
...
- **WHEN** 传入 `frontmatterMcpCapabilities = {knowledge_base: {domain: "knowledge-base"}}`，`installedMcpServers = ["dingtalk-wiki"]`，且有对应的 `registry.mcpTools` 条目
- **THEN** 返回 `{"knowledge_base":[{"id":"dingtalk-wiki","name":"钉钉云文档","skill":"mcp-dingtalk-wiki"}]}`
