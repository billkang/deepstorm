...
- **THEN** AI SHALL 读取 `.claude/skills/mcp-dingtalk-wiki/SKILL.md` 了解工具调用方式
- **THEN** AI SHALL 按指南将 PRD Markdown 推送到钉钉云文档
- **THEN** 成功后 `publishChecklist[0]` 记录 `{step:"knowledge_base_push", done:true}`
- **THEN** 成功后 `services.knowledgeBase` 记录 `{provider:"dingtalk-wiki", url:"..."}`
...
- **THEN** AI SHALL 读取 `.claude/skills/mcp-jira/SKILL.md` 了解工具调用方式
