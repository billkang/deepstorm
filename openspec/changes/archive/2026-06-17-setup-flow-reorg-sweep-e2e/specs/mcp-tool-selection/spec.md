...
- **SHALL** 仅生成 `mcpServers` 的 MCP 服务（如 Jira、GitHub、Playwright）才写入 `.mcp.json`；仅 env 类服务（如 Context7）仅写入 `.env` stub 和 guide 输出
...
- **THEN** Step 2 展示 reef 关联的 MCP 列表：Jira、GitHub、Figma、钉钉云文档、Context7
...
用户选择 MCP 服务后，系统应自动生成 `.mcp.json` 并写入 env stub。新增 Playwright MCP 支持，Playwright 使用 WebSocket 连接方式。
...
- **SHALL** Playwright MCP 的 `url` 使用 `ws://` 协议，从环境变量 `❗已废弃的 Playwright 环境变量（地址现硬编码在 .mcp.json 中）` 读取
...
| Playwright | `❗已废弃的 Playwright 环境变量（地址现硬编码在 .mcp.json 中）` |

#### Scenario: 安装 Playwright MCP
- **WHEN** 用户选择了 Playwright MCP
- **THEN** `.mcp.json` 中增加 `deepstorm-playwright` MCP server 条目，使用 WebSocket 连接
- **AND** `.env` 中增加 `❗已废弃的 Playwright 环境变量（地址现硬编码在 .mcp.json 中）=ws://localhost:54321` stub
- **AND** `deepstorm.installedMcpServers` 包含 `["playwright"]`

#### Scenario: 已有 `.env` 时追加 Playwright
- **WHEN** `.env` 已经存在
- **AND** 用户新选择了 Playwright
- **THEN** `.env` 中追加 `❗已废弃的 Playwright 环境变量（地址现硬编码在 .mcp.json 中）=ws://localhost:54321` 一行
- **AND** 已有的环境变量行保持不变
...
- **THEN** 输出包含：
  ...
  ✔ 已安装 3 个外部服务
    ...
    ❗已废弃的 Playwright 环境变量（地址现硬编码在 .mcp.json 中）=   ← Playwright WebSocket Endpoint
  ```
