...
- **AND** Playwright MCP 连接地址通过 `❗已废弃的 Playwright 环境变量（地址现硬编码在 .mcp.json 中）` 环境变量配置（默认值 `ws://localhost:54321`）
...
#### Scenario: Playwright env stub 生成
- **WHEN** 用户选中了 Playwright MCP 服务
- **THEN** CLI SHALL 在 `.env` 中追加以下 stub：
  ```
  # DeepStorm Playwright MCP — 浏览器自动化
  # 获取方式：Playwright MCP 默认使用 ws://localhost:54321
  ❗已废弃的 Playwright 环境变量（地址现硬编码在 .mcp.json 中）=ws://localhost:54321
  ```
- **AND** 变量值默认为 `ws://localhost:54321`（非空占位）
