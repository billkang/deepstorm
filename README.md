...
| **Pilot** | 领航 | 套件（daemon + CLI） | OpenSpec 自动实现 Harness Agent |
| **CLI** | — | npm 包（`@deepstorm/cli`） | 一键安装向导、配置管理、模板管理、诊断 |

> **CLI 使用方式：** 全局安装 `npm install -g @deepstorm/cli` 后直接使用 `deepstorm` 命令，或通过 `npx @deepstorm/cli` 按需运行。
...
│       ├── skills/          # 运维技能
│       └── wizard.json      # 向导配置
│   └── pilot/         # 领航 — OpenSpec 自动实现 Agent
│       ├── src/             # TypeScript 源码
│       │   ├── cli/         # CLI 命令（run/status/log/stop/resume/list）
│       │   ├── daemon/      # 守护进程（orchestrator/claude-process/lock）
│       │   ├── monitor/     # 运行时监控（token/silence/dead-loop）
│       │   ├── retry/       # 重试策略（classifier/handler）
│       │   ├── state/       # 状态持久化（store/types）
│       │   └── config/      # 配置加载（schema/loader）
│       └── package.json
├── docs/
│   └── deepstorm.html                     # 设计哲学
├── .claude/
...
> **架构关键点：** skills/agents/hooks 运行时数据分布在各 package（tide/reef/sweep/atoll）中，pilot 作为独立 TypeScript 包提供守护进程和 CLI 功能。MCP 服务器配置集中存放在 `packages/cli/mcp/`（按 domain 分组），MCP 使用指南 skill 在 `packages/cli/mcp-skills/`。`pnpm build` 时由 `build-registry.ts` 聚合复制到 `dist/`，随 npm 包一同发布。
...
```bash
deepstorm init --name my-app --frontend angular --backend java
```
...
| 场景 | 全局安装 | 按需运行 |
|------|---------|---------|
| 安装 DeepStorm 套件 | `deepstorm setup` | `npx @deepstorm/cli setup` |
| 重新配置 | `deepstorm setup --reconfigure` | `npx @deepstorm/cli setup --reconfigure` |
| 创建新项目 | `deepstorm init` | `npx @deepstorm/cli init` |
| 诊断 | `deepstorm doctor` | `npx @deepstorm/cli doctor` |
...
> 全局安装后使用 `deepstorm`，按需运行使用 `npx @deepstorm/cli`。以下以 `deepstorm` 为例。

| 命令 | 用途 | 说明 |
|------|------|------|
| `deepstorm init` | 项目脚手架 | 初始化项目骨架 |
