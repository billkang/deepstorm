...
> **CLI 安装方式：** `npm install -g @deepstorm/cli` 全局安装后运行 `deepstorm setup`，或直接 `npx @deepstorm/cli setup`。安装向导自动选取匹配的 skills/agents/hooks 复制到 `.claude/skills/`，无需网络下载。
...
| **Python** | ≥ 3.10 | BMAD 依赖 |
| **uv** | 最新 | Python 包管理器 |
| **BMAD Method** | 最新 | Tide 的前置依赖，`npx bmad-method install` |

## 安装 CLI 工具

DeepStorm CLI (`@deepstorm/cli`) 作为 npm 包发布，无需克隆仓库即可安装使用。
...
|------|---------|------|
| **Node.js** | ≥ 20.12 | CLI 运行环境 |
| **pnpm** | ≥ 9 | 如果你只需使用 CLI（不参与开发），此为非必需 |
...
```bash
# 全局安装后，deepstorm 命令直接可用
npm install -g @deepstorm/cli
...
> 全局安装后可在任意目录下直接调用 `deepstorm` 命令，无需每次加 `npx`。两种方式效果一致。

安装向导会引导你完成以下步骤：

1. **选择套件** — 按需勾选 tide / reef / sweep / atoll
2. **配置 MCP 服务** — 勾选需要绑定的 MCP 服务（Jira、GitHub、Figma、飞书知识库）
3. **填写凭据** — 根据所选套件回答配置问题（也可 `.env` 延迟配置）
4. **确认模板** — 确认自动生成的模板变量值
5. **完成安装** — 自动将 skills/agents/hooks 复制到 `.claude/skills/`

安装完成后重启 Claude Code 会话，DeepStorm 套件即可生效。
...
运行后可见所有可用命令列表。此时 `.claude/` 中尚未安装任何 skills/agents/hooks，后续通过 `deepstorm setup` 命令按需添加。

### 开发者安装（克隆仓库）

如需参与 DeepStorm 套件开发或贡献代码，请克隆仓库并安装依赖：

```bash
git clone git@github.com:billkang/deepstorm.git
cd deepstorm
pnpm install
pnpm build
```

> **提示：** `pnpm build` 是必要的，因为各套件的 skills/agents/hooks 在 `packages/*/` 分布存储，需构建聚合到 `dist/` 后才能被 CLI 消费。
...
诊断命令会检查 CLI 版本、配置完整性、MCP 服务器连接状态等。

### 下一步

- 安装 BMAD Method（Tide 的前置依赖）：`npx bmad-method install`
- 查看各子项目文档获取详细使用说明：`cat packages/tide/README.md`

### CLI 命令一览

> 全局安装后 `deepstorm` 命令直接可用，也可用 `npx @deepstorm/cli` 临时调用。

| 命令 | 用途 | 说明 |
|------|------|------|
| `deepstorm setup` | 安装向导 | 交互式配置 + 复制 skills/agents/hooks |
| `deepstorm setup --reconfigure` | 重新配置 | 清空旧配置后重新运行向导 |
| `deepstorm setup --non-interactive` | 静默安装 | 通过 `--tools`/`--set` 参数传递配置 |
| `deepstorm config view` | 查看配置 | 输出当前 `deepstorm.*` 配置 |
| `deepstorm config set <key>=<value>` | 修改配置 | 修改单项配置（支持嵌套路径） |
| `deepstorm config reset` | 重置配置 | 清除所有 DeepStorm 配置 |
| `deepstorm config refresh` | 刷新模板 | 基于当前配置重新渲染所有 `.tmpl` 模板 |
| `deepstorm template list` | 模板列表 | 查看所有可用 skill 模板 |
| `deepstorm template init` | 导出模板 | 导出 skill 默认内容供用户修改 |
| `deepstorm template apply` | 应用模板 | 将用户修改的模板应用到 `.claude/skills/` |
| `deepstorm update` | 全量更新 | 检查 CLI 版本 + 同步已安装 skill 官方最新模板 |
| `deepstorm plugin build` | 构建插件 | 交互式构建 DeepStorm Claude Code 插件 |
| `deepstorm doctor` | 诊断 | 检查 CLI 配置、MCP 完整性等 |
| `deepstorm uninstall` | 卸载 | 清除所有 DeepStorm 生成的内容 |
...
# 方式二：直接调用已发布的 CLI（全局安装）
deepstorm plugin build
